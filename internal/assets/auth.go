...
// auth.go — admin auth middleware. assets-svc doesn't have its own
// session store; it asks dock to introspect Bearer tokens via
// /internal/v1/auth/verify (cached 30s in the SDK). Only callers
// whose role is "admin" can hit the /api/admin/assets-* surface.
...
func (p *Plugin) requireAdminViaDock() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !access.Enabled {
			c.AbortWithStatusJSON(http.StatusForbidden, gin.H{"error": "workspace not granted access to assets"})
			return
		}
		c.Next()
	}
